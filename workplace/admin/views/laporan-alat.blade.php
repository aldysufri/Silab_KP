...
@section('breadcrumb')
    ...
    <li><a href="{{ base_url('dashboard/laporan-alat') }}">Laporan</a><i class="fa fa-circle"></i></li>
    <span>Alat</span>
@endsection

@section('page-title', 'Laporan Data Alat')
	...
	<div class="portlet light bordered">
		<div class="portlet-title">
			<div class="caption font-green">
                ...
                <form action="{{ base_url('dashboard/filter-alat')}}" method="GET">
		...
		<div class="portlet-body">
			<table id="laporan" class="table table-striped table-bordered table-hover order-column dt-responsive" width="100%" cellspacing="0" width="100%">
				<thead>
                    <tr>
                        <th>No</th>
                        <th>Nama Alat</th>
				...
				<tbody>
                        @foreach ($laporan as $p => $alat)
                        <tr>
                                <td>{{$p+1}}</td>
                                <td>{{ $alat->nama_alat }}</td>
                                <td>{{ $alat->merk_alat }}</td>
                                <td>{{ $alat->stokawal . ' ' . $alat->satuan_alat }}</td>
                                <td>{{$alat->masuk ?? 0;}}</td>
                                <td>{{$alat->keluar ?? 0;}}</td>
                                <td>{{$alat->stokawal + $alat->masuk - $alat->keluar}}</td>
		...
		jQuery(function($) {
			...
			$('#laporan').dataTable({
				language: {
					...
					info: "Showing _tanggal_awal_ to _END_ of _TOTAL_ records",
