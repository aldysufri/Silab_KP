...
@section('page-title', 'Daftar Penjadwalan')
	...
	<div class="portlet light bordered">
		<div class="portlet-title">
			<div class="caption font-green">
				...
				<span class="caption-subject bold uppercase">List Daftar Penjadwalan</span>
			</div>
		</div>
			...
			<table id="table_peminjaman" class="table table-striped table-bordered table-hover order-column dt-responsive" width="100%" cellspacing="0" width="100%">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{ date('H:i', strtotime($kegiatan->jam_kegiatan)) }}</td>
						<td>{{ $kegiatan->ruang_kegiatan }}</td>
						<td>
							<a class="btn btn-xs green" href='{{ base_url("dashboard/penjadwalan-edit/$kegiatan->id_kegiatan") }}' data-toggle="tooltip" data-placement="top" title="Edit"><i class="icon-note"></i></a>
							<a class="kegiatan-delete btn btn-xs red" data-id="{{ $kegiatan->id_kegiatan }}" data-toggle="tooltip" data-placement="top" title="Hapus"><i class="icon-trash"></i></a>
						</td>
					</tr>
					@endforeach
				</tbody>
...
@section('page-scripts')
	...
	<script type="text/javascript">
		...
		$('.kegiatan-delete').on('click', function() {
			var target = '{{ base_url('dashboard/penjadwalan-hapus') }}';
			var id = $(this).data('id');
